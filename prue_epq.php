	...
	<td valign="top">
		<center>
			...
			<?php
				...
				echo'<tr><td rowspan="5"><img src="img/epq.gif"></td></tr>';
				echo'<tr><td rowspan="5" width="10%"></td></tr>';				
				echo'<tr><td colspan="6"><b>ASPIRANTE: '. strtoupper($nom) .'</td></tr>';
				echo'<tr>';
				echo'<td width="10%" style=" border:1px solid #000000"><b>ESCALA</td>';
				echo'<td width="8%" style=" border:1px solid #000000"><b><center>N</td>';
				echo'<td width="8%" style=" border:1px solid #000000"><b><center>E</td>';
				echo'<td width="8%" style=" border:1px solid #000000"><b><center>P</td>';
				echo'<td width="8%" style=" border:1px solid #000000"><b><center>S</td>';
				echo'<td width="10%"></td>';
				echo'</tr>';
				echo'<tr>';
...
echo'</table>';
	            ...
	            echo'<th width="6%">ID.</td>';
       		    echo'<th width="80%">PREGUNTAS</td>';
	            echo'<th width="5%">RESPUESTAS</td>';
				echo'</tr></table>';
				echo'<div id="Layer1" style="width:700px; height:260px; overflow: scroll;">';
				echo'<table width="80%" border="0" cellspacing="0" cellpadding="0" class="tabla">';
									
$consul='select t1.idepq c1,t2.preguntas c2,t1.respuesta c3 from tb_respepq as t1 inner join tb_epq as t2 on t1.idepq=t2.idepq where t1.idaspirante="'. $cod .'" and t1.idnum_prue="'. $num_prue. '";';
				$result0 = mysql_query($consul, $conexion);
				$i=0;
...
<tr <?php if (($i%2)==0) {?>class="modo1" <?php }else {?> class="modo2" <?php } ?> onMouseOver="this.className='modo3'" onMouseOut="<?php if (($i%2)==0) {?>this.className='modo1' <?php } else { ?> this.className='modo2'<?php } ?> ">
			<?php	
					if($row['c3']==1){ $rr="Si";}elseif($row['c3']==0){ $rr="No";}else{$rr="-";}
					echo'<td width="5%">'.$row['c1'].'</td>';
			        echo'<td width="70%">'.utf8_encode($row['c2']).'</td>';
					echo'<td width="10%">'.$rr.'</td>';
			        echo'</tr>';
					$i++;
				}
			echo'</table>';
			echo'</div>';
			echo'<a href="rev_pruebaB.php?var='.$num_prue.'"><img src="img/volver.jpg" border="0" class="glossy"></a>';
			?>
			<!--FIN DEL CONTENIDO-->			
		</center>
	</td>
