<?php
	...
	if($_POST['bandera']=="ingresar")
	{
		...
		if ($archivo != "") 
		{
			$destino =  "files/Imp_".$prefijo."_".$archivo;
			if (copy($_FILES['archivo']['tmp_name'],$destino)) 
			{
				...
				{ 
					...
					if ($row>0)
					{
						if($row<10){$numfin="000".$row;}else if($row<100){$numfin="00".$row;}else if($row<1000){$numfin="0".$row;}else{$numfin=$row;}
 						$insertar="INSERT INTO tb_aspirantes(idaspirante,nit,nombre,apellido,profesorado,sexo,edad) VALUES ('$numfin', '$data[0]', '$data[1]', '$data[2]','$data[3]','$data[4]','$data[5]');"; 
						//echo $insertar.'<br>';
				...
				fclose ($fp);
				$status = "OK: Se a importado los datos con exito...";
			} 
			else 
			{
				$status = "ERROR: No se pudo importar los de datos...";
			}
		} 
		else 
		{
			$status = "ERROR: Debes de seleccionar el archivo con los datos a importar...";
		}
	}
?>
<html>
...
<tr> 
	<td valign="top"> 
		<center> <br />
			...
			<img src="img/impor.gif" border="0">
			...
			<tr>
				<td colspan="2">
					<center>
					<input type="button" name="Submit" value="Importar"  onclick= "res(Ffile)" class="button" />
					</center>				</td>
			</tr>
			<tr>
